    ...
    void)
{
    ...
    assert(text_size == strlen("Before start"));
    assert(scid_game_initial_comment_get(game, NULL, 0, &text_size) == SCID_ERROR_BUFFER_FULL);
    assert(text_size == strlen("Before start"));
    ...
    assert(scid_game_initial_comment_get(game, text, sizeof(text), NULL) == SCID_ERROR_BAD_ARG);
    assert(scid_game_tag_get(game, NULL, text, sizeof(text), &text_size) == SCID_ERROR_BAD_ARG);
    assert(scid_game_tag_get(game, "Event", text, sizeof(text), NULL) == SCID_ERROR_BAD_ARG);
