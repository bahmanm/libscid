        ...
        { // Test random read/write: write the remaining part of the data,
          // alternating with random-access read.
            ...
            while (nLeft > 0)
            {
                ...
                EXPECT_TRUE(std::equal(v2.begin(), v2.begin() + nRead, v.begin() + pos));
            }
        }
    ...
    read_write_uint32_t)
{
    ...
    { // Test random write
        ...
        for (int i = 0; i < 1000; ++i)
        {
            ...
            file.pubseekpos(v8.size() + v16.size() * 2 + v24.size() * 3 + pos * 4);
...
static const char* filename = SCID_TEST_RESOURCES_DIR "res_utf8.txt";
