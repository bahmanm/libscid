...
namespace scid::database
{
        ...
        const IndexEntry& ie)
    {
        ...
        std::copy_n(ie.GetHomePawnData(), info.homePawnData.size(), info.homePawnData.begin());
        ...
        std::size_t scidFlagsLen) const
    {
        ...
        if (!ie)
            return scid::core::ERROR_BadArg;
        game_storage::loadStandardTags(dest, scidFlags, scidFlagsLen, *ie, tagRoster(*ie));
        return scid::core::OK;
    }
        ...
        scid::core::uint& ply) const
    {
        ...
        if (useVariations)
        {
            ...
            if (ply == 0 && possibleMatch && game_search::varExactMatch(game, pos, searchType))
            {
                ply = matchedPly;
            }
            if (ply == 0 && possibleFlippedMatch &&
        ...
        scid::core::uint& ply) const
    {
        ...
        if (!ie)
            return scid::core::ERROR_BadArg;
        return searchBoard(
            *ie, game, pos, posFlip, useVariations, possibleMatch, possibleFlippedMatch, searchType,
            ply);
    }
        ...
        int maxDiff) const
    {
        ...
        if (possibleMatch)
        {
            ...
            result = game_search::materialMatch(
                hasPromotion, dataClone, min, max, patterns, patternCount, minPly, maxPly,
                matchLength, oppBishops, sameBishops, minDiff, maxDiff);
        }
        if (!result && possibleFlippedMatch)
        {
            result = game_search::materialMatch(
                hasPromotion, data, minFlipped, maxFlipped, flippedPatterns, flippedPatternCount,
                minPly, maxPly, matchLength, oppBishops, sameBishops, minDiff, maxDiff);
        }
        return result;
    }
        ...
        int maxDiff) const
    {
        ...
        return materialSearchMatch(
            *ie, possibleMatch, possibleFlippedMatch, min, max, minFlipped, maxFlipped, patterns,
            patternCount, flippedPatterns, flippedPatternCount, minPly, maxPly, matchLength,
        ...
        std::vector<std::string_view> const& removeTags)
    {
        ...
        for (const auto gnum : hfilter)
        {
            ...
            encodeBuf.insert(encodeBuf.end(), gamedata.data(), gamedata.data() + gamedata.size());
            err = saveGameData(ie, tagRoster(ie), {encodeBuf.data(), encodeBuf.size()}, gnum);
            if (err != scid::core::OK)
                break;
        ...
        gamenumT gNum)
    {
        ...
        if (const auto data =
                srcBase->storage_->codec->getGameData(ie->GetOffset(), ie->GetLength()))
     ...
     * Statistics
     */
    const scidBaseT::Stats&
    scidBaseT::getStats() const
    {
        if (stats_ == NULL)
        ...
        const HFilter& filter) const
    {
        ...
        for (gamenumT gnum = 0, n = numGames(); gnum < n; gnum++)
        {
            ...
            auto it = std::find_if(
                res.begin(), res.end(), [move](auto const& stat) { return stat.move == move; });

            auto& node = (it != res.end()) ? *it : res.emplace_back(move);
            node.add(ie->GetResult(), ie->GetWhiteElo(), ie->GetBlackElo(), ie->GetYear());
        }
        ...
        const Progress& progress)
    {
        ...
        if (auto err_Create =
                tmp.openHelper(codecName(dbtype), FMODE_Create, tmp_filename.string().c_str()))
            return err_Create;
