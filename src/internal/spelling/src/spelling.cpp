...
namespace scid::spelling
{
        ...
        const char* s)
    {
        ...
        v.push_back(
            std::make_pair(
                std::string(s + parse[0], parse[1]), std::string(s + parse[2], parse[3])));
        return scid::core::OK;
    }
        ...
        scid::core::dateT date) const
    {
        ...
        if (n == 0)
            return 0; // No data for that year
    ...
    std::string
    PlayerElo::isValid() const
    {
        for (size_t i = 1, n = elo_.size(); i < n; i++)
        {
            if (elo_[i].first < elo_[i - 1].first)
                return "unsorted";
        }

        auto count = [this](scid::core::uint year) {
            return std::count_if(
                this->elo_.begin(), this->elo_.end(),
                [&](const std::pair<uint16_t, scid::core::ratingT>& e) { return e.first == year; });
        };
        ...
        std::vector<const char*>* bio) const
    {
        ...
        if (it == idx_[scid::database::NAME_PLAYER].end())
            return 0; // not found

        if (bio != 0)
            *bio = pInfo_[it->idx].bio_;
        ...
        const char* prefix) const
    {
        ...
        {
            if (res.second->alias.compare(0, s.length(), s) != 0)
                break;
            if (res.second->alias == s)
                ...
                const Parser& data)
            {
                ...
                switch (data.type)
                {
                    ...
                    case SPELL_ALIAS:
                        ...
                        {
                            sp_.idx_[nt_].push_back(
                                SpellChecker::Idx(
                                    sp_.normalizeAndTransform(nt_, data.name), nameIdx_));
                        }
                        return scid::core::OK;
                    case SPELL_PREFIX:
        ...
        for (size_t lineStart = 0; lineStart < fileBuffer.size();)
        {
            ...
            const auto nextLine =
                lineEnd == fileBuffer.end()
                    ? fileBuffer.size()
                    : static_cast<size_t>(std::distance(fileBuffer.begin(), lineEnd)) + 1;
            ...
            {
                if (!progress.report(
                        static_cast<size_t>(report_done), static_cast<size_t>(fileSize)))
    ...
    PlayerInfo::getTitle() const
    {
        static const char* titles[] = {
            "GM", "IM", "FM", "WGM", "WIM", "WFM", "W", "CGM", "CIM", "HGM", NULL};
        ...
        while (*titlePtr != NULL)
        {
            if (scid::database::strIsCasePrefix(*titlePtr, comment))
            {
                return *titlePtr;
            }
            titlePtr++;
