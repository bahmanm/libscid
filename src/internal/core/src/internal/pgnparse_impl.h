    ...
    class PgnVisitor
    {
        ...
        public:
            ...
            visitPGN_inputUnexpectedPGNHeader()
            {
                if (nErrorsAllowed_)
                    logErr(
                        "Unexpected end of game: PGN header '[' seen "
            ...
            visitPGN_Comment(TView comment)
            {
                ...
                [[maybe_unused]] const bool updated = setCurrentMoveComment(game, str, location_);
                assert(updated);
                return true;
            }
            ...
            visitPGN_EPD(TView line)
            {
                ...
                {
                    auto opcode =
                        std::find_if(line.first, line.second, [spaces = 0](char ch) mutable {
        ...
        private:
                ...
                TView              rating)
            {
                ...
                {
                    game.setWhiteRating({static_cast<scid::core::ratingT>(elo), rType});
                }
                else
                {
                    game.setBlackRating({static_cast<scid::core::ratingT>(elo), rType});
                }
                return res;
            }
            ...
            parsedTagValue(std::string_view tag) const
            {
                ...
                if (tag == "ECO" && !coreGame.eco().empty())
                    return coreGame.eco();
                if (tag == "EventDate" && coreGame.eventDate() != scid::core::ZERO_DATE)
                {
                ...
                TView       value)
            {
                switch (tagLen)
                {
                    case 3:
                        if (std::equal(tag, tag + 3, "ECO"))
                        {
                            game.setEco(
                                {value.first,
                                 static_cast<std::size_t>(value.second - value.first)});
                            return true;
                        }
                ...
                if (tagLen >= 8)
                {
                    if (std::equal(tag, tag + 5, "White") && game.white().rating.value == 0)
                    {
                        auto res = parseRating(scid::core::WHITE, tag + 5, tagLen - 5, value);
                        if (res >= 0)
                            return res;
                    }
                    else if (std::equal(tag, tag + 5, "Black") && game.black().rating.value == 0)
                    {
