...
namespace scid::core
{
        ...
        std::string_view initialComment)
    {
        ...
        parents_.push_back({currentLine_, nextIndex_, move->childVariations.size() - 1});
    ...
    MovetextCursor::promoteVariationToMainline()
    {
        ...
        auto selectedVariation = std::move(rootMove.childVariations[parent.variationIndex]);
        rootMove.childVariations.erase(rootMove.childVariations.begin() + parent.variationIndex);
        ...
        demotedMainline.initialComment = std::move(selectedVariation.initialComment);
