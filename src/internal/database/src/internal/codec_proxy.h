    ...
    template <typename Derived> class CodecProxy : public CodecMemory
    {
        ...
        private:
            ...
            addName(
                ...
                const char*) final
            {
                return std::pair<scid::core::errorT, idNumberT>(
                    scid::core::ERROR_CodecUnsupFeat, 0);
            }
                ...
                NameBase*       nb) final
            {
                ...
                scid::core::errorT err =
                    CodecMemory::dyn_open(FMODE_Create, filename, progress, idx, nb);
                if (err != scid::core::OK)
                    return err;
                ...
                return parseGames(
                    progress, *getDerived(), [&](scid::core::Game& game, const char* scidFlags) {
                        buf.clear();
                ...
                TDestFn          destFn)
            {
                ...
                std::thread producer([&]() {
                    ...
                    while (true)
                    {
                        ...
                        int sy;
                        while (true)
                        { // spinlock if the slot is in use
                        ...
                        if (src.parseNext(
                                game[slot], scidFlags[slot].data(), scidFlags[slot].size()) ==
                            scid::core::ERROR_NotFound)
                            break;

                        if (++nProduced % 1024 == 0)
                        {
                            workDone.store(src.parseProgress().first, std::memory_order_release);
                        }
                ...
                while (true)
                {
                    ...
                    if (++nImported % 1024 == 0)
                    {
                        if (!progress.report(workDone.load(std::memory_order_acquire), workTotal))
                        {
