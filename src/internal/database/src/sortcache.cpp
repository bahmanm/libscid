...
namespace scid::database
{
        ...
        const char*     criteria)
    {
        ...
        static const char fields[] = {
            SORTING_date,         SORTING_year,       SORTING_event,    SORTING_site,
            SORTING_round,        SORTING_white,      SORTING_black,    SORTING_eco,
            SORTING_result,       SORTING_moveCount,  SORTING_avgElo,   SORTING_country,
            SORTING_deleted,      SORTING_eventdate,  SORTING_whiteelo, SORTING_blackelo,
            SORTING_commentcount, SORTING_varcount,   SORTING_nagcount, SORTING_resultwin,
            SORTING_resultdraw,   SORTING_resultloss, SORTING_rating,   SORTING_number,
            SORTING_sentinel};
    ...
    SortCache::checkForChanges(gamenumT id)
    {
        ...
        else
        {
            ...
            if (valid_fullMap_)
            {
                ...
                {
                    std::rotate(std::upper_bound(begin, it, *it, comp), it, it + 1);
                }
                else if (it + 1 != end && comp(*(it + 1), *it))
                {
                    std::rotate(it, it + 1, std::lower_bound(it + 1, end, *it, comp));
                }
                ASSERT(std::is_sorted(begin, end, comp));
            }
            else
        ...
        for (const char* field = criteria_; *field != SORTING_sentinel; field += 2)
        {
            ...
            switch (*field)
            {
                ...
                case SORTING_round:
                {
                    ...
                    res = (id1 == id2) ? 0
                                       : strCompareRound(
                                             nbase_->GetName(NAME_ROUND, id1),
                                             nbase_->GetName(NAME_ROUND, id2));
                    break;
                }

                case SORTING_resultwin:
                    res = (ie1->GetResult() == scid::core::RESULT_White ? 1 : 0) -
                          (ie2->GetResult() == scid::core::RESULT_White ? 1 : 0);
                    break;

                case SORTING_resultdraw:
                    res = (ie1->GetResult() == scid::core::RESULT_Draw ? 1 : 0) -
                          (ie2->GetResult() == scid::core::RESULT_Draw ? 1 : 0);
                    break;

                case SORTING_resultloss:
                    res = (ie1->GetResult() == scid::core::RESULT_Black ? 1 : 0) -
                          (ie2->GetResult() == scid::core::RESULT_Black ? 1 : 0);
                    break;
