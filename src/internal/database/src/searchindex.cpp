...
namespace scid::database
{
    ...
    namespace
    {
            ...
            public:
                    ...
                    : base_(base)
                {
                    ...
                    while (*results != 0)
                    {
                        ...
                        if (it != end)
                            result_[std::distance(scid::core::RESULT_CHAR, it)] = true;
                        results++;
                    }
                }
        ...
        template <typename T> class SearchRange : public StrRange
        {
            protected:
                const scidBaseT* base_;
         ...
         *
         * Return: the new partition point so that the range [itB, result) contains the matching
         * games If @param is not found, invalidate it and return the previous partition point
         * (@itR)
         */
        template <typename I>
        I
            ...
            SearchParam& param)
        {
            if (param == "player")
                return std::stable_partition(
                    itB, itE,
                    SearchName(
                        base, param.getValue(), NAME_PLAYER, &IndexEntry::GetWhite,
                        &IndexEntry::GetBlack));
            if (param == "white")
                return std::stable_partition(
                    itB, itE,
            ...
            if (param == "site")
                return std::stable_partition(
                    itB, itE, SearchName(base, param.getValue(), NAME_SITE, &IndexEntry::GetSite));
            if (param == "sitecountry")
                return std::stable_partition(itB, itE, SearchSiteCountry(base, param.getValue()));
            if (param == "round")
                return std::stable_partition(
                    itB, itE,
            ...
            if (param == "date")
                return std::stable_partition(
                    itB, itE, SearchRangeDate(base, param.getValue(), &IndexEntry::GetDate));
            if (param == "eventdate")
                return std::stable_partition(
                    itB, itE, SearchRangeDate(base, param.getValue(), &IndexEntry::GetEventDate));
            if (param == "elo")
                return std::stable_partition(
                    itB, itE,
                    SearchRangeElo(
                        base, param.getValue(), &IndexEntry::GetWhiteElo,
                        &IndexEntry::GetBlackElo));
            if (param == "welo")
                return std::stable_partition(
                    itB, itE, SearchRangeElo(base, param.getValue(), &IndexEntry::GetWhiteElo));
            if (param == "belo")
                return std::stable_partition(
                    itB, itE, SearchRangeElo(base, param.getValue(), &IndexEntry::GetBlackElo));
            if (param == "delo")
                return std::stable_partition(
                    itB, itE,
                    SearchRangeEloDiff(
            ...
            if (param == "eco")
                return std::stable_partition(
                    itB, itE, SearchRangeEco(base, param.getValue(), &IndexEntry::GetEcoCode));
            if (param == "gnum")
                return std::stable_partition(itB, itE, SearchRangeGamenum(base, param.getValue()));
            if (param == "length")
                return std::stable_partition(
                    itB, itE,
                    SearchRange<scid::core::ushort>(
                        base, param.getValue(), &IndexEntry::GetNumHalfMoves));
            if (param == "n_variations")
                return std::stable_partition(
                    itB, itE,
                    SearchRange<scid::core::uint>(
                        base, param.getValue(), &IndexEntry::GetVariationCount));
            if (param == "n_comments")
                return std::stable_partition(
                    itB, itE,
                    SearchRange<scid::core::uint>(
                        base, param.getValue(), &IndexEntry::GetCommentCount));
            if (param == "n_nags")
                return std::stable_partition(
                    itB, itE,
                    SearchRange<scid::core::uint>(
                        base, param.getValue(), &IndexEntry::GetNagCount));
            if (param == "flag")
                return std::stable_partition(itB, itE, SearchFlag(base, param.getValue()));
            if (param == "result")
                return std::stable_partition(itB, itE, SearchResult(base, param.getValue()));
            if (param == "variant")
                return std::stable_partition(itB, itE, SearchVariant(base, param.getValue()));
     ...
     * means (white name contains 'carlsen' || black name contains 'carlsen') && (black name
     * contains 'kramnik')
     *
     ...
     * and will find the games played by players with elo greater or equal than 2700, excluding the
     * blitz events
     *
     * -welo "2700 4000" -belo|! "0 2700" -delo "-200 200"
     * means (white elo > 2700 && white elo < 4000) || (belo is not in the range 0-2700) && ((welo -
     * belo) > -200 && (welo - belo) < 200)
     */
    scid::core::errorT
    search_index(
        const scidBaseT* base,
