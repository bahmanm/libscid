    ...
    void)
{
    ...
    assert(value == 0);
    assert(scid_movetext_cursor_variation_index_get(cursor, &value) == SCID_OK);
    assert(value == 0);
    ...
    assert(truth == 0);
    assert(scid_movetext_cursor_is_line_start(cursor, &truth) == SCID_OK);
    assert(truth == 1);
    assert(scid_movetext_cursor_is_line_end(cursor, &truth) == SCID_OK);
    assert(truth == 0);
    assert(scid_movetext_cursor_is_variation_start(cursor, &truth) == SCID_OK);
    assert(truth == 1);
    assert(scid_movetext_cursor_is_variation_end(cursor, &truth) == SCID_OK);
    assert(truth == 0);
    ...
    assert(count == 1);
    assert(scid_movetext_cursor_next_move_nag_at_get(cursor, 0, &nag) == SCID_OK);
    assert(nag == 1);
    ...
    assert(scid_movetext_cursor_previous_movespec_get(cursor, &move) == SCID_OK);
    assert(move.from == 12);
    assert(move.to == 28);
    assert(move.promotion == SCID_PIECE_NONE);
    assert(move.is_castling == 0);
