...
namespace
{
    ...
    testFilePath(std::string_view stem)
    {
        ...
        path /= std::string(stem) + "_" +
                std::to_string(::testing::UnitTest::GetInstance()->random_seed()) + ".eco";
        return path;
    }
...
TEST_F(
    EcoBookTest,
    MissingFileReturnsOpenError)
{
    auto book = scid::eco::Book::load(path_);

    ASSERT_FALSE(book);
    EXPECT_EQ(scid::eco::ERROR_FileOpen, book.error());
}

TEST_F(
    EcoBookTest,
    LoadIndexesPositionsAndClassifiesKnownLines)
{
    writeFile(path_, kEcoFile);

    auto book = scid::eco::Book::load(path_);
    ASSERT_TRUE(book) << book.error();
...
TEST_F(
    EcoBookTest,
    LinesWithPrefixReturnsStructuredRows)
{
    writeFile(path_, kEcoFile);
...
TEST_F(
    EcoBookTest,
    CorruptFileReturnsCorruptError)
{
    ...
    ASSERT_FALSE(book);
    EXPECT_EQ(scid::eco::ERROR_Corrupt, book.error());
}
