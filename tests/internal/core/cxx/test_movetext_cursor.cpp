...
namespace
{
        ...
        AddsMainlineMovesAtCursorAndAdvances)
    {
        ...
        first.san = "e4";
        auto& second = cursor.addMove(quiet(scid::core::E7, scid::core::E5));
        second.san = "e5";
    ...
    TEST(
        CoreMovetextCursorTest,
        AddsVariationToNextMoveAndEntersIt)
    {
        scid::core::Game game;
        ...
        auto& variationMove = cursor.addMove(quiet(scid::core::D2, scid::core::D4));
        variationMove.san = "d4";

        auto const& first = game.movetext().mainline.moves[0];
        ASSERT_EQ(1U, first.childVariations.size());
        EXPECT_EQ("Queen pawn alternative", first.childVariations[0].initialComment);
        ...
        SetsCurrentVariationInitialComment)
    {
        ...
        EXPECT_EQ(
            "New comment", game.movetext().mainline.moves[0].childVariations[0].initialComment);
    }
        ...
        TruncatesMainlineAtCursor)
    {
        ...
        EXPECT_EQ("e2e4", game.movetext().mainline.moves[0].spec.longNotation());
    }
        ...
        DeletesCurrentVariationAndExitsToParent)
    {
        ...
        ASSERT_NE(nullptr, cursor.nextMove());
        EXPECT_EQ("e2e4", cursor.nextMove()->spec.longNotation());
        auto const& variations = game.movetext().mainline.moves[0].childVariations;
