...
namespace
{
    ...
    scid::database::fileModeT fmodes[] = {
        scid::database::FMODE_Create, scid::database::FMODE_ReadOnly,
        scid::database::FMODE_WriteOnly, scid::database::FMODE_Both};
    const char* filename = "codecbase";

    scid::database::CodecType codecs[] = {
        scid::database::CodecType::Memory, scid::database::CodecType::Scid4,
    ...
    std::vector<std::pair<scid::database::CodecType, std::string>> unsupportedVec = {
        ...
        {scid::database::CodecType::Memory,
         "FMODE" + std::to_string(scid::database::FMODE_ReadOnly)},
        {scid::database::CodecType::Memory,
         "FMODE" + std::to_string(scid::database::FMODE_WriteOnly)},
        ...
        public:
                ...
                const scid::database::Index& idx)
            {
                ...
                for (auto& game : encoded)
                {
                    ...
                    EXPECT_TRUE(
                        std::equal(data.data(), data.data() + entry->GetLength(), game.data()));
                }
            }
            ...
            genGame()
            {
                ...
                for (auto i = rand(0, maxMoves); i > 0; --i)
                {
                    ...
                    position->GenerateMoves(
                        &mlist, scid::core::EMPTY, scid::core::GEN_ALL_MOVES, true);
                    if (mlist.Size() == 0)
                        break;

                    auto move = *mlist.Get(rand(0, mlist.Size() - 1));
                    addMove(
                        *res, location, {move.from, move.to, move.promote, move.isCastle() != 0});
