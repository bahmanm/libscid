    ...
    select_sortedPosition)
{
    ...
    auto sort_vIndex = [&vIndex, nb, &getFieldIdx](std::string criteria) {
        sort(vIndex.begin(), vIndex.end(), [&](auto& a, auto& b) { return a[0] < b[0]; });
        ...
        while (it != criteria.rend())
        {
            ...
            stable_sort(vIndex.begin(), vIndex.end(), [&](auto& a, auto& b) {
                ...
                {
                    auto strA = nb->GetName(
                        scid::database::NAME_SITE,
                        static_cast<scid::database::idNumberT>(a[field]));
                    auto strB = nb->GetName(
                        scid::database::NAME_SITE,
    ...
    auto test_listGames = [&dbase, &buf, &vIndex](auto crit, auto flt, size_t start, size_t count) {
        EXPECT_EQ(count, dbase.listGames(crit, start, buf.size(), flt, buf.data()));
        EXPECT_TRUE(
            std::equal(
                buf.begin(), buf.begin() + count, vIndex.begin() + start,
    ...
    auto test_sortedPosition = [&](auto crit, auto flt) {
        ...
        auto it =
            std::find_if(vIndex.begin(), vIndex.end(), [id](auto& rec) { return id == rec[0]; });
    ...
    for (int pass = 0; pass < 2; ++pass)
    {
        for (auto& criteria : testCrit)
        {
            ...
            bool validCriteria = (getFieldIdx(*crit) != -1) && (crit[1] == '+' || crit[1] == '-');
            ...
            for (auto flt : {dbfilter, filter, emptyfilter})
            {
                ...
                if (validCriteria)
                {
                    stable_partition(vIndex.begin(), vIndex.end(), [flt](auto& e) {
