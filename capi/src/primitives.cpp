...
{
    return abi_guard(0, [&]() -> int { return status == SCID_WARNING_NAME_DATA_LOSS ? 1 : 0; });
}
...
{
    return abi_guard(
        0, [&]() -> int { return (status != SCID_OK && !scid_is_warning(status)) ? 1 : 0; });
}
    ...
    scid_square* out_square)
{
    ...
    *out_square = 0;
    ...
    return abi_guard([&]() -> scid_error { return parse_square(text, out_square); });
}
