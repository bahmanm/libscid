...
namespace scid::core
{
    ...
    namespace
    {
            ...
            pieceT* board)
        {
            ...
            unique_sq.erase(std::unique(unique_sq.begin(), unique_sq.end()), unique_sq.end());
            auto unique_col = std::vector<pieceT>(n);
            std::transform(begin, begin + n, unique_col.begin(), [&](auto sq) {
                return piece_Color(board[sq]);
            });
            std::sort(unique_col.begin(), unique_col.end());
            unique_col.erase(std::unique(unique_col.begin(), unique_col.end()), unique_col.end());
            auto kings = std::vector<pieceT>(n);
            std::transform(
                begin, begin + n, kings.begin(), [&](auto sq) { return piece_Type(board[sq]); });
        ...
        TFunc   not_empty) const
    {
        ...
        const auto rpawn_atk = square_Move(target_sq, enemy == WHITE ? DOWN_RIGHT : UP_RIGHT);
        ...
        for (size_t i = 1, n = GetCount(enemy); i < n; ++i)
        {
            ...
            {
                if (move_predicates::attack_slider(piece_sq, target_sq, piece_type, not_empty))
    ...
    Position::canCastle(bool king_side) const
    {
        ...
        const squareT rookTo =
            king_side ? square_Relative(ToMove, F1) : square_Relative(ToMove, D1);
        const squareT kingTo =
            king_side ? square_Relative(ToMove, G1) : square_Relative(ToMove, C1);
    ...
    template bool
    Position::canCastle<false>(bool king_side) const;
...
#define POSSIBLE_CAPTURE(d)                                                                        \
    ((d != NULL_SQUARE) && ((piece_Color(Board[d]) == (color_Flip(ToMove))) ||                     \
        ...
        squareT rsq)
    {
        ...
        Castling |= 1u << idx;
        const auto std_rsq = (dir == QSIDE) ? square_Relative(col, A1) : square_Relative(col, H1);
    ...
    Position::getStdStart()
    {
        ...
        if (init)
        {
            ...
            for (uint i = 0; i < 8; i++)
            {
                p->AddToBoard(WP, A2 + i);
                p->List[WHITE][i + 8] = A2 + i;
                p->ListPos[A2 + i] = i + 8;
                p->AddToBoard(BP, A7 + i);
    ...
    bool
    Position::IsStdStart() const
    {
        if (ToMove != WHITE || Hash != stdStartHash || PawnHash != stdStartPawnHash ||
            GetCount(WHITE) != 16 || GetCount(BLACK) != 16 || RankCount(WP, RANK_2) != 8 ||
            RankCount(BP, RANK_7) != 8 || RankCount(WN, RANK_1) != 2 ||
            RankCount(BN, RANK_8) != 2 || !GetCastling(WHITE, KSIDE) ||
            !GetCastling(WHITE, QSIDE) || !GetCastling(BLACK, KSIDE) || !GetCastling(BLACK, QSIDE))
        {
            return false;
        }
        return true;
        ...
        bool      maybeInCheck)
    {
        ...
        {
            mask = (1 << KING) | (1 << QUEEN) | (1 << ROOK) | (1 << BISHOP) | (1 << KNIGHT) |
                   (1 << PAWN);
        }
        ...
        for (uint x = 1; x < npieces; x++)
        {
            ...
            { // piece x is pinned to king
                ...
                else
                {
                    ...
                    if (ptype == QUEEN || (ptype == ROOK && !direction_IsDiagonal(pinned)) ||
                        (ptype == BISHOP && direction_IsDiagonal(pinned)))
                    {
                        GenSliderMoves(mlist, ToMove, sq, pinned, NULL, capturesOnly);
                        GenSliderMoves(mlist, ToMove, sq, dirOpposite[pinned], NULL, capturesOnly);
                    }
                }
            }
            else
        ...
        pieceT          promo)
    {
        ...
        if (!move_predicates::pseudo(
                from, to, toMove, pt, [&](auto sq) { return pos.GetPiece(sq) != EMPTY; }))
        ...
        if (pt == PAWN)
        {
            if (captured == EMPTY && square_Fyle(from) != square_Fyle(to))
            {
                ...
                if (to != pos.GetEPTarget() || pos.GetPiece(captured_sq) != piece_Make(enemy, PAWN))
            ...
            {
                if (promo != QUEEN && promo != ROOK && promo != BISHOP && promo != KNIGHT)
    ...
    Position::TreeCalcAttacks(squareT target)
    {
        ...
        auto it = std::partition(
            moves.begin(), moves.end(), [&](auto const& sm) { return sm.to == target; });
        ...
        SquareList* fromSquares) const
    {
        ...
        do
            if (Board[*destPtr] == king)
                fromSquares->Add(*destPtr);
        while (*++destPtr != NULL_SQUARE);
    ...
    Position::IsKingInCheck(MoveAction const& sm)
    {
        ...
        if (move_predicates::attack(sm.to, GetKingSquare(), color_Flip(ToMove), p, [&](auto sq) {
                return GetPiece(sq) != EMPTY;
            }))
            return true;
    ...
    {
        res.castling = 0;
        if (promo == EMPTY || promo == INVALID_PIECE)
        { // NORMAL MOVE
            res.from = from;
            res.to = to;
            res.promote = EMPTY;
    ...
    Position::apply(MoveAction const& sm)
    {
        ...
        if (ptype == PAWN)
        {
            ...
            if (fromRank == RANK_2 && toRank == RANK_4 &&
                (Board[square_Move(to, LEFT)] == BP || Board[square_Move(to, RIGHT)] == BP))
            {
                EPTarget = square_Move(from, UP);
            }
            if (fromRank == RANK_7 && toRank == RANK_5 &&
                (Board[square_Move(to, LEFT)] == WP || Board[square_Move(to, RIGHT)] == WP))
            {
        ...
        std::string* toSAN)
    {
        ...
        while (auto len = std::find_if(moves, end, is_space) - moves)
        {
            ...
            {
                sanStringT san;
                writeSan(sm, san, moves != end ? SAN_CHECKTEST : SAN_MATETEST);
                if (WhiteToMove())
                {
        ...
        fyleT       frFyle)
    {
        ...
        auto is_digit = [](char ch) { return isdigit(static_cast<unsigned char>(ch)); };
        auto is_lower = [](char ch) { return islower(static_cast<unsigned char>(ch)); };
        ...
        size_t      slen) const
    {
        ...
        auto not_empty = [&](auto sq) { return sq != from && GetPiece(sq) != EMPTY; };
    ...
    Position::PrintCompactStr(char* cboard) const
    {
        ...
        if (ToMove == WHITE)
        {
            if (Board[square_Move(ep, DOWN_LEFT)] != WP && Board[square_Move(ep, DOWN_RIGHT)] != WP)
            {
                ep = NULL_SQUARE;
            }
        }
        ...
        {
            if (Board[square_Move(ep, UP_LEFT)] != BP && Board[square_Move(ep, UP_RIGHT)] != BP)
            {
                ep = NULL_SQUARE;
            }
        }
    ...
    Position::ReadFromFEN(const char* str)
    {
        ...
        skip_spaces();
        str = FEN_parsePieces(
            str, [&](auto piece, auto sq) { return this->AddPiece(piece, sq) == OK; });
        if (!str)
        ...
        size_t len) const
    {
        ...
        if (validCastlingFlag(WHITE, true))
        {
            ...
            fen.push_back(
                isChess960() && rook_sq != find_castle_rook(WHITE, H1)
        ...
        if (validCastlingFlag(WHITE, false))
        {
            ...
            fen.push_back(
                isChess960() && rook_sq != find_castle_rook(WHITE, A1)
                    ? static_cast<char>(toupper(square_FyleChar(rook_sq)))
                    : 'Q');
        }
        if (validCastlingFlag(BLACK, true))
        {
            ...
            fen.push_back(
                isChess960() && rook_sq != find_castle_rook(BLACK, H8) ? square_FyleChar(rook_sq)
                                                                       : 'k');
        }
        if (validCastlingFlag(BLACK, false))
        {
            ...
            fen.push_back(
                isChess960() && rook_sq != find_castle_rook(BLACK, A8) ? square_FyleChar(rook_sq)
                                                                       : 'q');
        }
