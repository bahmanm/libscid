...
namespace scid::database
{
    ...
    namespace
    {
            ...
            NameBase&   nb)
        {
            ...
            for (nameT nt : {NAME_PLAYER, NAME_EVENT, NAME_SITE, NAME_ROUND})
            {
                ...
                for (idNumberT i = 0; i < Header_numNames[nt]; i++)
                {
                    ...
                    { // Frequencies all <= 255: fit in one scid::core::byte
                        file.ReadOneByte();
                    }
                    // ***
                    ...
                    if (id >= Header_numNames[nt] || !nb.insert(prevName.c_str(), length, nt, id))
                        return scid::core::ERROR_Corrupt;
                }
            }
            ...
            const TFreq& freq)
        {
            ...
            for (nameT nt : {NAME_PLAYER, NAME_EVENT, NAME_SITE, NAME_ROUND})
            {
                ...
                for (const auto& it : names_ids[nt])
                {
                    ...
                    ASSERT(
                        prevName == nullptr ||
                        static_cast<scid::core::uint>(*prevName) <
                            static_cast<scid::core::uint>(*name) ||
                        (static_cast<scid::core::uint>(*prevName) ==
                             static_cast<scid::core::uint>(*name) &&
        ...
        constexpr size_t INDEX_HEADER_SIZE = 8 + 2 + 3 + 4 + 3 + SCID_DESC_LENGTH + 1 +
                                             (CUSTOM_FLAG_DESC_LENGTH + 1) * CUSTOM_FLAG_MAX;
            ...
            HeaderT& header)
        {
            ...
            if (!std::equal(
                    std::begin(magic), std::end(magic), std::begin(INDEX_MAGIC),
                    std::end(INDEX_MAGIC)))
            {
                return {scid::core::ERROR_BadMagic, {}};
            }
            ...
            header.description.assign(desc, std::find(desc, desc + SCID_DESC_LENGTH, '\0'));
            ...
            gamenumT       nGames)
        {
            ...
            char desc[SCID_DESC_LENGTH + 1] = {};
            std::copy_n(
                Header.description.data(), std::min(Header.description.size(), SCID_DESC_LENGTH),
                desc);
            ...
            for (auto const& flagDesc : Header.flagDesc)
            {
                char buf[CUSTOM_FLAG_DESC_LENGTH + 1] = {};
                std::copy_n(
                    flagDesc.data(), std::min(flagDesc.size(), CUSTOM_FLAG_DESC_LENGTH), buf);
            ...
            IndexEntry* ie)
        {
            ...
            else
            {
                ...
                edate = DATE_MAKE(
                    eyear, scid::core::date_GetMonth(edate), scid::core::date_GetDay(edate));
            }
            ...
            ie->SetHomePawnData(pawnData0 & 0x3F, reinterpret_cast<const unsigned char*>(buf_it));
        }
            ...
            char*             buf_it)
        {
            ...
            else
            {
                ...
                edate = (eyear << 9) | (scid::core::date_GetMonth(edate) << 5) |
                        scid::core::date_GetDay(edate);
            }
            ...
            static_assert(
                ...
                std::numeric_limits<decltype(ie->GetStoredLineCode())>::max() < (1ULL << 8));
        ...
        NameBase*       nb)
    {
        ...
        if (fMode == FMODE_Create)
        {
            ...
            {
                err =
                    namefileWrite(filenames_[1].c_str(), nb_->getNames(), nb->calcNameFreq(*idx_));
            }
        }
        else
        {
            ...
            if (header_.version < SCID_OLDEST_VERSION || header_.version > SCID_VERSION)
    ...
    CodecSCID4::flush()
    {
        ...
        scid::core::errorT errSync =
            (idxfile_.pubsync() != 0) ? scid::core::ERROR_FileWrite : scid::core::OK;
        ...
        if (err == scid::core::OK)
        {
            ...
            // recalculation.
            err = namefileWrite(filenames_[1].c_str(), nb_->getNames(), nb_->calcNameFreq(*idx_));
        }
        scid::core::errorT errGfile =
            (gfile_.pubsync() == 0) ? scid::core::OK : scid::core::ERROR_FileWrite;
        ...
        Progress const& progress)
    {
        ...
        const auto nBytes = (header_.version < 400) ? OLD_INDEX_ENTRY_SIZE : INDEX_ENTRY_SIZE;
        ...
        const char* new_value)
    {
        ...
        else
        {
            ...
            if (flag < IndexEntry::IDX_FLAG_CUSTOM1 || flag > IndexEntry::IDX_FLAG_CUSTOM6)
                return scid::core::ERROR_CodecUnsupFeat;
