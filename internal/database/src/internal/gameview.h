    ...
    class GameView
    {
        private:
            ...
            class MaterialCount
            {
                ...
                public:
                    ...
                    count(
                        scid::core::colorT color,
                        scid::core::pieceT piece_type) const
                    {
                        ASSERT(color == 0 || color == 1);
                        ASSERT(piece_type > 0 && piece_type < 8);
            ...
            class FastBoard
            {
                ...
                public:
                    ...
                    stdStart()
                    {
                        static const auto std_start =
                            FastBoard(scid::core::Position::getStdStart());
                        return std_start;
                    }
                    ...
                    countMaterial(const scid::core::pieceT* board)
                    {
                        ...
                        for (int i = 0; i < 64; ++i)
                        {
                            ...
                            {
                                mt_count.incr(
                                    scid::core::piece_Color_NotEmpty(board[i]),
                                    scid::core::piece_Type(board[i]));
                            }
                        }
                        return mt_count;
                    }
                            ...
                            for (uint8_t idx = 0; idx < 16; ++idx)
                            {
                                if (idx < pos_count)
                                {
                                    ...
                                    if (piece_type == scid::core::ROOK &&
                                        scid::core::square_Rank(sq) ==
                                            scid::core::rank_Relative(color, scid::core::RANK_1))
                                    {
                        ...
                        int                 newIdx = EMPTY_SQ_)
                    {
                        ...
                        {
                            scid::core::squareT moved_sq =
                                pieces_.remove(color, oldIdx, lastvalid_idx);
                ...
                private:
                        ...
                        scid::core::pieceT  lastPt) const
                    {
                        ...
                        for (int i = 1, n = mt_.count(lastCol); i < n; i++)
                        {
                            ...
                            if (!scid::core::move_predicates::pseudo(
                                    sq, lastTo, lastCol, lastPt, isOccupied))
                                continue; // Skip: illegal move

                            const auto pin = scid::core::move_predicates::opens_ray(
                                sq, lastTo, kingSq, isOccupied);
                            ...
                            if (scid::core::square_Rank(lastFrom) == scid::core::square_Rank(sq))
                            {
                                ...
                                scid::core::square_Fyle(lastFrom) == scid::core::square_Fyle(sq))
                            {
                                ambiguity |= 4; // 0b0100
                            }
                        }
                        ...
                        scid::core::colorT  color) const
                    {
                        for (int idx = 0, n = mt_.count(color); idx < n; ++idx)
                        {
                            ...
                            if (pt != scid::core::QUEEN && pt != scid::core::ROOK &&
                                pt != scid::core::BISHOP)
                                continue;
                            ...
                            if (scid::core::move_predicates::attack_slider(
                                    sq, destSq, pt, isOccupied))
                            {
                                return idx;
                            }
                        }
        ...
        public:
            ...
            getMove(int ply_to_skip)
            {
                ...
                {
                    auto move = (cToMove_ == scid::core::WHITE)
                                    ? DecodeNextMove<scid::core::FullMove, scid::core::WHITE>()
                                    : DecodeNextMove<scid::core::FullMove, scid::core::BLACK>();
                    if (!move)
                        break;
            ...
            search(const scid::core::pieceT* board)
            {
                ...
                for (;;)
                {
...

                    {
                        ...
                        if (less_material(board_.materialCount(), mt_count, 1 - toMove, move))
                            return 0;
                    }
                    {
                        const auto move = DecodeNextMove<scid::core::FullMove, 1 - toMove>();
                        if (less_material(board_.materialCount(), mt_count, toMove, move))
                            return 0;
                    }
