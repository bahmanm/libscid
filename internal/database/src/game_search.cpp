...
namespace scid::database
{
    ...
    namespace
    {
        ...
        int
        calcHomePawnMask(
            scid::core::pieceT        pawn,
            const scid::core::pieceT* board)
        {
            ASSERT(pawn == scid::core::WP || pawn == scid::core::BP);
            const scid::core::pieceT* bd =
                &(board[(pawn == scid::core::WP ? scid::core::H2 : scid::core::H7)]);
            int result = 0;
            if (*bd == pawn)
            {
                result |= 128;
            ...
            size_t                      patternCount)
        {
                 ...
                 pattern != patternEnd; ++pattern)
            {
                if (pattern->rankMatch == scid::core::NO_RANK)
                {
                    ...
                    if (pattern->fyleMatch == scid::core::NO_FYLE)
                    { // Nothing to test!
                    }
                    else
                    { // Test this fyle:
                        scid::core::squareT sq =
                            scid::core::square_Make(pattern->fyleMatch, scid::core::RANK_1);
                        bool found = false;
                        for (scid::core::uint i = 0; i < 8; i++, sq += 8)
                        {
                            if (board[sq] == pattern->pieceMatch)
                            {
                                found = true;
                                break;
                            }
                        }
                        if (found != (pattern->flag != 0))
                        {
                            return false;
                        }
                    }
                ...
                { // rankMatch is a rank from 1 to 8:
                    ...
                    { // Test the whole rank:
                        bool                found = false;
                        scid::core::squareT sq =
                            scid::core::square_Make(scid::core::A_FYLE, pattern->rankMatch);
                    ...
                    { // Just test one square:
                        scid::core::squareT sq =
                            scid::core::square_Make(pattern->fyleMatch, pattern->rankMatch);
                        bool found = false;
                        if (board[sq] == pattern->pieceMatch)
                        {
                            found = true;
                ...
                8>& blackPawnFyles)
        {
            if (searchPos->GetToMove() != currentPosition.GetToMove() ||
                searchPos->GetCount(scid::core::WHITE) !=
                    currentPosition.GetCount(scid::core::WHITE) ||
                searchPos->GetCount(scid::core::BLACK) !=
                    currentPosition.GetCount(scid::core::BLACK) ||
                searchPos->PieceCount(scid::core::WP) !=
                    currentPosition.PieceCount(scid::core::WP) ||
                searchPos->PieceCount(scid::core::BP) !=
                    currentPosition.PieceCount(scid::core::BP) ||
                searchPos->PieceCount(scid::core::WN) !=
                    currentPosition.PieceCount(scid::core::WN) ||
                searchPos->PieceCount(scid::core::BN) !=
                    currentPosition.PieceCount(scid::core::BN) ||
                searchPos->PieceCount(scid::core::WB) !=
                    currentPosition.PieceCount(scid::core::WB) ||
                searchPos->PieceCount(scid::core::BB) !=
                    currentPosition.PieceCount(scid::core::BB) ||
                searchPos->PieceCount(scid::core::WR) !=
                    currentPosition.PieceCount(scid::core::WR) ||
                searchPos->PieceCount(scid::core::BR) !=
                    currentPosition.PieceCount(scid::core::BR) ||
                searchPos->PieceCount(scid::core::WQ) !=
                    currentPosition.PieceCount(scid::core::WQ) ||
            ...
            {
                for (scid::core::squareT sq = scid::core::A1; sq <= scid::core::H8;
                     sq++, currentBoard++, searchBoard++)
                {
            ...
            {
                return fyleCountsMatch(currentPosition, whitePawnFyles, blackPawnFyles);
            }
                ...
                8>& blackPawnFyles)
        {
            for (auto const& move : line.moves)
            {
                ...
                {
                    if (varExactMatchLine(
                            variation.line, currentPosition, searchPos, searchType, whitePawnFyles,
                            blackPawnFyles))
                    {
                        return true;
                    }
        ...
        scid::core::Position
        startPositionFor(const scid::core::Game& game)
        {
            ...
            int               maxDiff)
        {
            ...
            while (err == scid::core::OK)
            {
                ...
                if (currentPosition.PieceCount(scid::core::WP) < min[scid::core::WP])
                {
                    return false;
                }
                if (currentPosition.PieceCount(scid::core::BP) < min[scid::core::BP])
                {
                    return false;
                }
                ...
                if (currentPosition.PieceCount(scid::core::WQ) < min[scid::core::WQ])
                {
                    goto Check_Promotions;
                }
                if (currentPosition.PieceCount(scid::core::BQ) < min[scid::core::BQ])
                {
                    goto Check_Promotions;
                }
                if (currentPosition.PieceCount(scid::core::WR) < min[scid::core::WR])
                {
                    goto Check_Promotions;
                }
                if (currentPosition.PieceCount(scid::core::BR) < min[scid::core::BR])
                {
                    goto Check_Promotions;
                }
                if (currentPosition.PieceCount(scid::core::WB) < min[scid::core::WB])
                {
                    goto Check_Promotions;
                }
                if (currentPosition.PieceCount(scid::core::BB) < min[scid::core::BB])
                {
                    goto Check_Promotions;
                }
                if (currentPosition.PieceCount(scid::core::WN) < min[scid::core::WN])
                {
                    goto Check_Promotions;
                }
                if (currentPosition.PieceCount(scid::core::BN) < min[scid::core::BN])
                {
                    goto Check_Promotions;
                }
                wMinor = currentPosition.PieceCount(scid::core::WB) +
                         currentPosition.PieceCount(scid::core::WN);
                bMinor = currentPosition.PieceCount(scid::core::BB) +
                         currentPosition.PieceCount(scid::core::BN);
                if (wMinor < min[scid::core::WM])
                {
                    goto Check_Promotions;
                }
                if (bMinor < min[scid::core::BM])
                {
                    goto Check_Promotions;
                }

                // Now test maximum counts:
                if (currentPosition.PieceCount(scid::core::WQ) > max[scid::core::WQ])
                {
                    goto Next_Move;
                }
                if (currentPosition.PieceCount(scid::core::BQ) > max[scid::core::BQ])
                {
                    goto Next_Move;
                }
                if (currentPosition.PieceCount(scid::core::WR) > max[scid::core::WR])
                {
                    goto Next_Move;
                }
                if (currentPosition.PieceCount(scid::core::BR) > max[scid::core::BR])
                {
                    goto Next_Move;
                }
                if (currentPosition.PieceCount(scid::core::WB) > max[scid::core::WB])
                {
                    goto Next_Move;
                }
                if (currentPosition.PieceCount(scid::core::BB) > max[scid::core::BB])
                {
                    goto Next_Move;
                }
                if (currentPosition.PieceCount(scid::core::WN) > max[scid::core::WN])
                {
                    goto Next_Move;
                }
                if (currentPosition.PieceCount(scid::core::BN) > max[scid::core::BN])
                {
                    goto Next_Move;
                }
                if (currentPosition.PieceCount(scid::core::WP) > max[scid::core::WP])
                {
                    goto Next_Move;
                }
                if (currentPosition.PieceCount(scid::core::BP) > max[scid::core::BP])
                {
                    goto Next_Move;
                }
                if (wMinor > max[scid::core::WM])
                {
                    goto Next_Move;
                }
                if (bMinor > max[scid::core::BM])
                {
                    goto Next_Move;
                }
                    ...
                    currentPosition.PieceCount(scid::core::BB) == 1)
                {
                    ...
                    { // Check the restriction:
                        ...
                        ASSERT(
                            blackBishCol != scid::core::NOCOLOR &&
                            whiteBishCol != scid::core::NOCOLOR);
                ...
                if (patternCount == 0 || patternsMatch(&currentPosition, patterns, patternCount))
                {
                    foundMatch = true;
                    matchesNeeded--;
                    if (matchesNeeded <= 0)
            ...
            gameExactMatchT         searchType)
        {
            ...
            if (searchType == GAME_EXACT_MATCH_Exact || searchType == GAME_EXACT_MATCH_Pawns)
            {
            ...
            while (err == scid::core::OK)
            {
                ...
                if (searchPos->GetCount(scid::core::WHITE) >
                        currentPosition->GetCount(scid::core::WHITE) ||
                    searchPos->GetCount(scid::core::BLACK) >
                        currentPosition->GetCount(scid::core::BLACK))
                {
                ...
                if (searchPos->PieceCount(scid::core::WP) >
                        currentPosition->PieceCount(scid::core::WP) ||
                    searchPos->PieceCount(scid::core::BP) >
                        currentPosition->PieceCount(scid::core::BP))
                {
                ...
                if (searchPos->PieceCount(scid::core::WP) !=
                    ...
                    searchPos->PieceCount(scid::core::WQ) !=
                        currentPosition->PieceCount(scid::core::WQ) ||
                    searchPos->PieceCount(scid::core::BQ) !=
                        currentPosition->PieceCount(scid::core::BQ))
                {
                    // skip++;
                    goto Move_Forward;
                }
                ...
                else if (searchType == GAME_EXACT_MATCH_Fyles)
                {
                    ...
                    {
                        if (searchPos->FyleCount(scid::core::WP, f) !=
                                currentPosition->FyleCount(scid::core::WP, f) ||
                            searchPos->FyleCount(scid::core::BP, f) !=
                                currentPosition->FyleCount(scid::core::BP, f))
                        {
                            found = false;
                            break;
                        }
            ...
            Move_Forward:
            {
                ...
                if (buf == NULL)
                {
                    if (memoryLine == nullptr || memoryMoveIndex >= memoryLine->moves.size())
                    {
                ...
                {
                    err = game_storage::decodeMainlineMove(*buf, *currentPosition, nextMove);
                }
        ...
        int               maxDiff)
    {
        return materialMatches(
            promotionsFlag, buf, min, max, patterns, patternCount, minPly, maxPly, matchLength,
            oppBishops, sameBishops, minDiff, maxDiff);
    }


    bool
    game_search::exactMatch(
        const scid::core::Game& game,
        ...
        gameExactMatchT         searchType)
    {
        ...
        return varExactMatchLine(
            game.movetext().mainline, startPosition, pos, searchType, whitePawnFyles,
            blackPawnFyles);
    }
