    ...
    is_PGNsymbol)
{
    ...
    for (int i = 0; i < 256; i++)
    {
        EXPECT_EQ(chars[i], scid::core::pgn_impl::is_PGNsymbol(static_cast<signed char>(i)));
        EXPECT_EQ(chars[i], scid::core::pgn_impl::is_PGNsymbol(static_cast<unsigned char>(i)));
    }
}
