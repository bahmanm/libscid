    ...
    clone)
{
    for (auto filename : {gameUTF8, gameLatin1, gameLatin1Conv})
    {
        ...
        ASSERT_EQ(
            scid::core::OK,
            dbase.loadGame(*dbase.getIndexEntry(0), game, scidFlags.data(), scidFlags.size()));
        scid::core::MovetextLocation location;
        ...
        ASSERT_TRUE(std::equal(board, board + 66, clonePosition->GetBoard()));
    ...
    locationInPGN)
{
    for (auto filename : {gameUTF8, gameLatin1, gameLatin1Conv})
    {
        ...
        scid::core::Game game;
        game.clear();
        ASSERT_EQ(scid::core::OK, dbase.loadGameMovesOnly(*dbase.getIndexEntry(0), game));
        scid::core::MovetextLocation currentLocation;
...
TEST(
    Test_Game,
    coreGamePgnEncodingIncludesLegacyMetadataTags)
{
    ...
    currentPositionUci_startpos)
{
    ...
    for (auto [pos, str] : expected)
    {
        ...
        EXPECT_EQ(str, scid::core::notation::currentPositionUci(game, location));
    }
}

TEST(
    Test_Game,
    coreGameMovetextMirrorsLegacyMoveTree)
{
    std::string_view pgn = "1.d4! {Best by test} ({Queen pawn alternative} 1.e4 e5 ( 1...c5)) "
                           "(1.c4) 1...d5 2.c4";
    scid::core::Game game;
    scid::core::pgn::ParseLog log;
    ASSERT_TRUE(scid::core::pgn::parseGame(pgn.data(), pgn.size(), game, log));
    ...
    EXPECT_EQ("Queen pawn alternative", mainline[0].childVariations[0].initialComment);
    ...
    coreGameMovetextMirrorsProgrammaticVariationAdds)
{
    ...
    addMove(game, location, makeCurrentMove(game, location, scid::core::D2, scid::core::D4));
    ...
    coreGameMoveMetadataMirrorsProgrammaticNagMutation)
{
    ...
    auto const& firstNags = game.movetext().mainline.moves[0].metadata.nags;
    ...
    auto const& afterRemove = game.movetext().mainline.moves[0].metadata.nags;
    ASSERT_EQ(1U, afterRemove.size());
    EXPECT_EQ(scid::core::Nag::Equal, afterRemove[0]);

    clearCurrentNags(game, location);
    EXPECT_TRUE(game.movetext().mainline.moves[0].metadata.nags.empty());
}
    ...
    coreGameVariationMetadataMirrorsProgrammaticNagMutation)
{
    ...
    auto const& variationMove = game.movetext().mainline.moves[0].childVariations[0].line.moves[0];
    ASSERT_EQ(1U, variationMove.metadata.nags.size());
    EXPECT_EQ(scid::core::Nag::InterestingMove, variationMove.metadata.nags[0]);
}

TEST(
    Test_Game,
    coreGameMirrorsProgrammaticCommentMutation)
{
    using namespace std::literals;
    ...
    EXPECT_EQ("Queen pawn alternative"sv, mainlineMove.childVariations[0].initialComment);

    auto const& variationMove = mainlineMove.childVariations[0].line.moves[0];
    ...
    moveCommentReadsCoreCommentAtCurrentLocation)
{
    ...
    EXPECT_EQ("Core initial comment", std::string(currentMoveComment(game, location)));
    ...
    EXPECT_EQ("Core previous move comment", std::string(currentMoveComment(game, location)));

    addVariation(game, location);
    scid::core::MovetextCursor variationCursor(game);
    ASSERT_TRUE(variationCursor.restore(location));
    ASSERT_TRUE(variationCursor.setCurrentVariationInitialComment("Core variation comment"));
    EXPECT_EQ("Core variation comment", std::string(currentMoveComment(game, location)));
}
...
TEST(
    Test_Game,
    coreGameMirrorsInitialMovetextComment)
{
    using namespace std::literals;
...
TEST(
    Test_Game,
    coreGameCanBeEncodedAsPlainPgnWithoutStoredSan)
{
    using namespace std::literals;
    ...
    illegalSCID4_Castling)
{
    ...
    auto chess960 = make_invalid(
        ...
        "8.Nf5");
    EXPECT_EQ(decode_game(chess960), "b3 Ng6 e4 e5 Ng3 Nc6 f3 Bg5 Be2 a6 Nc3 d6 Nd5 O-O-O Nf5 ");
    EXPECT_EQ(
        decode_gameview(chess960), "1.b3 Ng6  2.e4 e5  3.Ng3 Nc6  4.f3 Bg5  5.Be2 a6  6.Nc3 d6  "
                                   "7.Nd5 O-O-O  8.Nf5");
    ...
    auto obstacles = make_invalid(
        9, // replace 4.h4 with O-O-O
        "1.d4 d5 2.Qd3 Nf6 3.Bg5 Nc6 {_} 4.h4");
    EXPECT_EQ(decode_game(obstacles), "d4 d5 Qd3 Nf6 Bg5 Nc6 O-O-O ");
    EXPECT_EQ(decode_gameview(obstacles), "1.d4 d5  2.Qd3 Nf6  3.Bg5 Nc6  4.O-O-O");
    ...
    EXPECT_EQ(decode_gameview(check), "1.d4 d5  2.Nf3 e6  3.e3 Nf6  4.Nc3 Be7  5.Bb5+ O-O");
    ...
    EXPECT_EQ(
        decode_gameview(twice), "1.e4 e5  2.Nf3 Nf6  3.Be2 Be7  4.O-O O-O  5.O-O-O a5  6.Kh1");
    ...
    EXPECT_EQ(decode_gameview(captured_rook), "1.Qa6 c5  2.Qxa8+ Qd8  3.Qc6+ Qd7  4.O-O");
    ...
    EXPECT_EQ(
        decode_gameview(occ_king), "1.e4 e5  2.-- Nf6  3.Be2 Be7  4.O-O O-O  5.Kh1 a5  6.Nf3");
    ...
    EXPECT_EQ(decode_gameview(occ_rook), "1.e4 e5  2.Nf3 Nf6  3.Be2 --  4.O-O O-O  5.Kh1");
}

TEST(
    Test_Game,
    illegalSCID4_KingCapture)
{
    auto data = make_invalid(
        0, // null move
