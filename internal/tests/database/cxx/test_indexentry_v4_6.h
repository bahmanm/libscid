...
class IndexEntry
{
        ...
        uint16_t Length_Low; // Length of gamefile record for this game. 17 bits are used so the max
                             // length is 128 ko (131071). So 7 bits are usable for custom flags or
                             // other.
        scid::core::byte
            Length_High; // LxFFFFFF ( L = length for long games, x = spare, F = custom flags)
    ...
    public:
        ...
        GetEventDate() const
        {
            ...
            year = dyear + year - 4;
            return (year << scid::core::YEAR_SHIFT) | (month << scid::core::MONTH_SHIFT) | day;
        }
        scid::core::resultT
        GetResult() const
        {
        ...
        scid::core::ratingTypeT
        GetWhiteRatingType() const
        {
            return u16_high_4(WhiteElo);
        }
        scid::core::ratingTypeT
        GetBlackRatingType() const
        {
            return u16_high_4(BlackElo);
        }
        ...
        GetStartFlag() const
        {
            return (Flags & (1 << IDX_FLAG_START)) != 0;
        }
        bool
        GetPromotionsFlag() const
        {
            return (Flags & (1 << IDX_FLAG_PROMO)) != 0;
        }
        bool
        GetUnderPromoFlag() const
        {
            return (Flags & (1 << IDX_FLAG_UPROMO)) != 0;
        }
        bool
        GetCommentsFlag() const
        {
            return (GetCommentCount() > 0);
        }
        bool
        GetVariationsFlag() const
        {
        ...
        {
            return DecodeCount(VarCounts & 15);
        }
        scid::core::uint
        GetCommentCount() const
        {
            return DecodeCount((VarCounts >> 4) & 15);
        }
        scid::core::uint
        GetNagCount() const
        {
        ...
        {
            return u32_low_24(FinalMatSig);
        }
        scid::core::byte
        GetStoredLineCode() const
        {
            return u32_high_8(FinalMatSig);
        }
        const scid::core::byte*
        GetHomePawnData() const
        {
            return HomePawnData;
        }
        scid::core::byte*
...
IndexEntry::CharToFlagMask(char flag)
{
    ...
    {
        case 'S':
            return 1 << IDX_FLAG_START;
        case 'X':
            return 1 << IDX_FLAG_PROMO;
        case 'Y':
            return 1 << IDX_FLAG_UPROMO;
        case 'D':
            return 1 << IDX_FLAG_DELETE;
        case 'W':
            return 1 << IDX_FLAG_WHITE_OP;
        case 'B':
            return 1 << IDX_FLAG_BLACK_OP;
        case 'M':
            return 1 << IDX_FLAG_MIDDLEGAME;
        case 'E':
            return 1 << IDX_FLAG_ENDGAME;
        case 'N':
            return 1 << IDX_FLAG_NOVELTY;
        case 'P':
            return 1 << IDX_FLAG_PAWN;
        case 'T':
            return 1 << IDX_FLAG_TACTICS;
        case 'K':
            return 1 << IDX_FLAG_KSIDE;
        case 'Q':
            return 1 << IDX_FLAG_QSIDE;
        case '!':
            return 1 << IDX_FLAG_BRILLIANCY;
        case '?':
            return 1 << IDX_FLAG_BLUNDER;
        case 'U':
            return 1 << IDX_FLAG_USER;
        case '1':
            return 1 << IDX_FLAG_CUSTOM1;
        case '2':
            return 1 << IDX_FLAG_CUSTOM2;
        case '3':
            return 1 << IDX_FLAG_CUSTOM3;
        case '4':
            return 1 << IDX_FLAG_CUSTOM4;
        case '5':
            return 1 << IDX_FLAG_CUSTOM5;
        case '6':
            return 1 << IDX_FLAG_CUSTOM6;
    }
