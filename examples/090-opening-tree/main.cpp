...
std::optional<scid::database::gamenumT> find_lasker_capablanca(
...
int main( int argc, char** argv )
{
    ...
    if( db.loadGame( *target_game, game, scid_flags.data(), scid_flags.size() )
        != scid::core::OK ) {
        return 1;
    }
    ...
    for( const auto& node : tree ) {
        std::cout << node.move.getSAN()
                  << " games=" << node.freq[ 0 ]
                  << " score=" << node.score() / 10.0
                  << "% results="
                  << node.freq[ scid::core::RESULT_White ] << '/'
