    ...
    const char* san)
{
    ...
    if (!check(scid_movetext_cursor_move_add(cursor, move), "scid_movetext_cursor_move_add"))
    {
        return 0;
    }
    ...
    void)
{
    ...
    if (!add_san_move(cursor, position, "e4") ||
        !check(
            scid_movetext_cursor_comment_set(cursor, "King pawn"),
            "scid_movetext_cursor_comment_set") ||
        !check(scid_movetext_cursor_nag_add(cursor, 1, &changed), "scid_movetext_cursor_nag_add") ||
        !changed || !add_san_move(cursor, position, "e5") ||
        !add_san_move(cursor, position, "Nf3") || !add_san_move(cursor, position, "Nc6"))
    {
        scid_movetext_cursor_free(cursor);
        scid_position_free(position);
        scid_game_free(game);
    ...
    if (!check(scid_movetext_cursor_to_ply(cursor, 1, &moved), "scid_movetext_cursor_to_ply") ||
        !moved ||
        !check(
            scid_movetext_cursor_position_get(cursor, position),
            "scid_movetext_cursor_position_get") ||
        !check(
            scid_movetext_cursor_variation_add(cursor, "Sicilian branch", &changed),
            "scid_movetext_cursor_variation_add") ||
        !changed || !add_san_move(cursor, position, "c5") ||
        !check(
            scid_movetext_cursor_variation_exit(cursor, &moved),
            "scid_movetext_cursor_variation_exit") ||
        !moved || !check(scid_movetext_cursor_to_end(cursor), "scid_movetext_cursor_to_end") ||
