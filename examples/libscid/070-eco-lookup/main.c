...
static int play(scid_position* position, const char* san) {
...
int main(void) {
    ...
    if (!check(scid_eco_book_load(LIBSCID_EXAMPLE_ECO_FILE, &book),
               "scid_eco_book_load") ||
        ...
        !play(position, "c4") ||
        !play(position, "e6") ||
        !check(scid_position_to_fen(position, fen, sizeof(fen), &fen_size),
               "scid_position_to_fen") ||
        !check(scid_eco_book_code_find(book, position, &code),
        ...
        !check(scid_eco_book_name_find(book, position, name, sizeof(name), &name_size),
