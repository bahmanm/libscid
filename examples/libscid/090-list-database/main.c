...
static int add_pgn(
    ...
    if (!check(scid_database_game_add(database, game, flags),
...
static int print_game_row(scid_database* database, size_t index) {
    char event[128];
    char white[128];
    char black[128];
    char date[32];
    ...
    if (!check(scid_database_game_number_get(database, index, &number),
               "scid_database_game_number_get") ||
        !check(scid_database_game_deleted_get(database, index, &deleted),
        ...
        !check(scid_database_game_tag_get(
                   ...
                   &black_size),
               "scid_database_game_tag_get") ||
        !check(scid_database_game_date_get(
                   database,
                   index,
                   date,
                   sizeof(date),
        ...
        !check(scid_database_game_eco_get(database, index, &eco),
               "scid_database_game_eco_get") ||
        !check(scid_eco_code_to_string(
                   eco,
                   SCID_ECO_FORMAT_BASIC,
                   eco_text,
                   sizeof(eco_text),
                   &eco_size),
               "scid_eco_code_to_string") ||
        !check(scid_database_game_halfmove_count_get(database, index, &halfmoves),
...
int main(void) {
    ...
    if (!check(scid_database_create_memory("listing", &database),
               "scid_database_create_memory") ||
        !add_pgn(database, first_pgn, "D") ||
        !add_pgn(database, second_pgn, "M") ||
    ...
    if (!check(scid_database_game_tag_get(
                   database,
                   0,
                   "Event",
                   text,
        ...
        !check(scid_database_game_deleted_get(database, 0, &deleted),
               "scid_database_game_deleted_get") ||
        deleted != 1 ||
        !check(scid_database_game_tag_get(
                   database,
