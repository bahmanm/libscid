...
int main(void) {
    ...
    if (!check(scid_database_create_memory("pgn-io", &database),
               "scid_database_create_memory") ||
        !check(scid_database_import_pgn(
                   database,
                   pgn,
                   strlen(pgn),
                   diagnostic,
        ...
        !check(scid_database_game_export_pgn(
                   database,
                   0,
                   exported,
                   sizeof(exported),
        ...
        !contains(exported, "[Event \"Database PGN\"]") ||
        !contains(exported, "1-0")) {
