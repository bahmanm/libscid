...
static int print_next_move(scid_movetext_cursor* cursor) {
    ...
    if (!check(scid_movetext_cursor_ply_get(cursor, &ply),
        ...
        !read_text(scid_movetext_cursor_next_move_san_get(
                   ...
                   san,
                   san_size) ||
        !read_text(scid_movetext_cursor_next_move_comment_get(
                       cursor,
                       comment,
                       sizeof(comment),
                       &comment_size),
                   ...
                   comment,
                   comment_size) ||
        !check(scid_movetext_cursor_next_move_nag_count_get(cursor, &nag_count),
               "scid_movetext_cursor_next_move_nag_count_get") ||
        !check(scid_movetext_cursor_variation_count_get(cursor, &variation_count),
    ...
    if (nag_count > 0) {
        if (!check(scid_movetext_cursor_next_move_nag_at_get(cursor, 0, &nag),
...
int main(void) {
    ...
    if (!read_text(scid_movetext_cursor_comment_get(
                       cursor,
                       text,
                       sizeof(text),
                       &text_size),
                   ...
                   text,
                   text_size) ||
        !text_equals(text, text_size, "Before game") ||
        !print_next_move(cursor) ||
        !check(scid_movetext_cursor_variation_count_get(cursor, &variation_count),
               "scid_movetext_cursor_variation_count_get") ||
        variation_count != 1 ||
        !check(scid_movetext_cursor_variation_enter(cursor, 0, &moved),
               "scid_movetext_cursor_variation_enter") ||
        !moved ||
        !check(scid_movetext_cursor_variation_depth_get(cursor, &depth),
               "scid_movetext_cursor_variation_depth_get") ||
        depth != 1 ||
        !read_text(scid_movetext_cursor_comment_get(
                       cursor,
                       text,
                       sizeof(text),
                       &text_size),
        ...
        !moved ||
        !check(scid_movetext_cursor_next(cursor, &moved),
               "scid_movetext_cursor_next") ||
        !moved ||
        !read_text(scid_movetext_cursor_previous_move_san_get(
                       cursor,
                       text,
                       sizeof(text),
                       &text_size),
