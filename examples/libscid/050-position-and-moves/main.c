...
int main(void) {
    const char* expected_fen =
        "rnbqkbnr/pp1ppppp/8/2p5/4P3/8/PPPP1PPP/"
        "RNBQKBNR w KQkq - 0 2";
    ...
    if (!check(scid_position_create_standard(&position),
               "scid_position_create_standard") ||
        !check(scid_movespec_create_from_san(position, "e4", &move),
               "scid_movespec_create_from_san") ||
        !check(scid_movespec_to_uci(move, text, sizeof(text), &text_size),
               "scid_movespec_to_uci") ||
        !text_equals(text, text_size, "e2e4") ||
        !check(scid_position_apply_san(position, "e4"),
               "scid_position_apply_san") ||
        !check(scid_position_apply_uci(position, "c7c5"),
               "scid_position_apply_uci") ||
        !check(scid_position_to_fen(position, text, sizeof(text), &text_size),
    ...
    if (!check(scid_position_side_to_move_get(position, &side_to_move),
               "scid_position_side_to_move_get") ||
        side_to_move != SCID_WHITE ||
        !check(scid_position_fullmove_number_get(position, &number),
               "scid_position_fullmove_number_get") ||
        number != 2 ||
        !check(scid_position_halfmove_clock_get(position, &number),
               "scid_position_halfmove_clock_get") ||
        number != 0 ||
        !check(scid_square_from_string("e4", &move.to),
               "scid_square_from_string") ||
        !check(scid_position_piece_at_get(position, move.to, &piece),
               "scid_position_piece_at_get") ||
        piece != SCID_PIECE_WHITE_PAWN ||
        !check(scid_square_to_string(move.to, text, sizeof(text), &text_size),
