...
int main(void) {
    ...
    if (!check(scid_database_create_memory("editing", &database),
               "scid_database_create_memory") ||
        !parse_pgn(original_pgn, &original) ||
        !parse_pgn(replacement_pgn, &replacement) ||
        !check(scid_database_game_add(database, original, NULL),
    ...
    if (!check(scid_database_game_replace(database, 0, replacement, "Q"),
    ...
    printf("after replace: %.*s %.*s flags=%.*s\n",
           (int)event_size,
           event,
           (int)result_size,
           result,
