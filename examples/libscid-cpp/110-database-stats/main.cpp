    ...
    char** argv)
{
    ...
    const auto result_total =
        stats.nResults[scid::core::RESULT_None] + stats.nResults[scid::core::RESULT_White] +
        stats.nResults[scid::core::RESULT_Black] + stats.nResults[scid::core::RESULT_Draw];
    ...
    const auto average_year =
        stats.nYears == 0 ? 0 : static_cast<unsigned>(stats.sumYears / stats.nYears);
    const auto average_rating =
        stats.nRatings == 0 ? 0 : static_cast<unsigned>(stats.sumRatings / stats.nRatings);

    std::cout << "Games: " << db.numGames() << '\n'
              << "Dates: " << date_to_string(stats.minDate) << " .. "
              << date_to_string(stats.maxDate) << '\n'
              << "Average year: " << average_year << '\n'
              << "Results: " << scid::core::RESULT_LONGSTR[scid::core::RESULT_White] << '='
